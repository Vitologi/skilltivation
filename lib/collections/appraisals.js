...
Meteor.methods({
    ...
    deleteAppraisal: function(parameters){
        ...
        return Appraisals.remove(id);
    },

    /**
     * Remove appraisal by source's id
     *
     * @param {Object} parameters       - appraisal attributes
     * @param {String} parameters.id    - source's id
     * @returns {*}
     */
    deleteSourceAppraisals: function(parameters){
        if(!this.userId)return false;
        ...
        if(!id)return Debugger.error(null, "Wrong arguments for removing appraisals by source's id");

        return Appraisals.remove({"userId":this.userId, "sourceId":id});
    }
